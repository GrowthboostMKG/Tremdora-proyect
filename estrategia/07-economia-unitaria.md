# 7. Economía unitaria y métricas

## 7.1 Los eBooks: margen enorme, conversión difícil

Con producto digital no hay coste de producto ni envío. El IVA de los libros electrónicos es del **4 %** (confirma con tu gestor que tus recursos califican como "libro" — ver doc. 08).

| Por pedido | Solo front-end | **Con bump y upsell** |
|---|---|---|
| Precio de venta | 22,00 € | AOV real **28,15 €** |
| IVA repercutido (4 %) | −0,85 € | −1,08 € |
| Comisiones de pago | −0,58 € | −0,67 € |
| **Margen de contribución** | **20,57 €** | **26,40 €** |
| **CAC máximo (break-even)** | **20,57 €** | **26,40 €** |

**El bump y el upsell suben tu CAC asumible un 28 %.** Y no te cuestan nada: son productos que ya tienes hechos.

## 7.2 Proyección de octubre (realista)

Con 1.200 € repartidos 28 €/día en venta y 12 €/día en captación:

| | Ruta A (venta) | Ruta B (muestra gratis) |
|---|---|---|
| Inversión | 840 € | 360 € |
| CPM estimado | 8 € | 8 € |
| Clics | ~1.365 | — |
| CVR de landing | 2,0 % | — |
| **Resultado** | **~27 compradores** | **~300 leads** |
| CPA / CPL | **31 €** | **1,20 €** |
| Margen generado | 713 € | — |
| **Balance** | **−127 €** | — |

**Octubre queda a break-even, y eso es el resultado correcto.** No estás comprando beneficio: estás comprando 27 compradores reales de nicho mascotas, 300 leads y —lo más valioso— **un píxel entrenado con compras reales**, que es lo que hará eficientes los anuncios de noviembre.

## 7.3 La conversación incómoda: la lista sola no sostiene la Navidad

Seamos honestos con el número:

```
27 compradores × 15 % de conversión  =   4 cajas
300 leads      ×  3 % de conversión  =   9 cajas
                                        ─────────
                                        ~13 cajas   ≈ 900 €
```

**Eso no es un lanzamiento de Navidad.** Es un ensayo.

Conclusión operativa: la preventa **necesita presupuesto de anuncios propio en noviembre**. La lista y el píxel no sustituyen la inversión — la hacen rentable. Planifica noviembre con:
- **Audiencia caliente** (lista + retargeting + compradores): CAC ≈ 0-8 €
- **Lookalike 1 % de compradores** (el píxel de octubre es lo que lo hace posible): CAC 15-30 €
- **Frío con el mejor creativo de octubre**: solo si las dos anteriores rinden

## 7.4 Cuadro de mando — 7 números, cada lunes

| # | Métrica | Objetivo octubre | Objetivo noviembre |
|---|---|---|---|
| 1 | Hook rate | > 25 % | > 30 % |
| 2 | CTR de enlace | > 1,3 % | > 1,8 % |
| 3 | CVR de landing | > 2,0 % | > 2,5 % |
| 4 | CPA de eBook | < 31 € | < 24 € |
| 5 | AOV | > 28 € | > 30 € |
| 6 | Coste por lead | < 1,50 € | < 1,20 € |
| 7 | **Compradores acumulados** | **> 30** | **— (pasa a nº de cajas)** |

> **En octubre tu métrica reina no es el ROAS: es el nº 7.** Cada comprador es una unidad de munición para noviembre.

## 7.5 Reglas de decisión

**Matar un anuncio:**
- Gasto ≥ 40 € sin ninguna compra
- Hook rate < 15 % con 2.000+ impresiones
- CTR < 0,8 % con 2.000+ impresiones
- **CTR bien y CVR < 0,8 %** → no es el anuncio, es la landing. No lo apagues: arregla la página.

**Escalar:** CPA < 24 € tres días seguidos → +20 % de presupuesto, esperar 48 h. Nunca más de un 20-30 % diario ni escalar y cambiar creatividad el mismo día.

**Tocar la cuenta:** un cambio cada 72 h como máximo. Con 40 €/día, todo dato de menos de tres días es ruido.

## 7.6 El árbol de los tres diagnósticos

```
¿Hook rate < 20 %?         → El problema es EL PRIMER SEGUNDO. 5 hooks nuevos.
¿Hook bien, CTR < 1 %?     → El problema es EL ÁNGULO. No conecta con el dolor.
¿CTR bien, CVR < 1 %?      → El problema es LA LANDING O EL PRECIO.
```

Nunca cambies las tres cosas a la vez.
